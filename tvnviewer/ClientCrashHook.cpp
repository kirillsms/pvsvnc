...
bool ClientCrashHook::m_guiEnabled = true;
HKEY ClientCrashHook::m_rootHkey = HKEY_CURRENT_USER;
LocalMutex ClientCrashHook::m_guiEnabledMutex;
ApplicationCrashEvents *ClientCrashHook::m_notifier = 0;

ClientCrashHook::ClientCrashHook(ApplicationCrashEvents *notifier)
{
  m_notifier = notifier;
  ::SetUnhandledExceptionFilter(topLevelExceptionFilter);
}

ClientCrashHook::~ClientCrashHook()
{
}

LONG WINAPI ClientCrashHook::topLevelExceptionFilter(_EXCEPTION_POINTERS *pExceptionInfo)
{
  bool guiEnabled;
  {
  ...
  try {
    ...
    try {
      ...
      RegistryKey regKey(root, RegistryPaths::VIEWER_PATH);
...
void ClientCrashHook::setGuiEnabled()
{
  AutoLock al(&m_guiEnabledMutex);
  m_guiEnabled = true;
}

void ClientCrashHook::setHklmRoot()
{
  AutoLock al(&m_guiEnabledMutex);
