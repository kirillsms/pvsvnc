...
HRESULT CAVIGenerator::InitEngine()
{
	...
	if (wVer < 0x010a)
	{
		...
		_tcscpy_s(m_sError,_T("Version of Video for Windows too old. Come on, join the 21th century!"));
		return S_FALSE;
	}
	...
	if (hr != AVIERR_OK)
	{
		...
		_tcscpy_s(m_sError,szBuffer);
		// Check it succeded.
		switch(hr)
		{
		case AVIERR_BADFORMAT: 
			_tcscat_s(m_sError,_T("The file couldn't be read, indicating a corrupt file or an unrecognized format."));
			break;
		case AVIERR_MEMORY:		
			_tcscat_s(m_sError,_T("The file could not be opened because of insufficient memory.")); 
			break;
		case AVIERR_FILEREAD:
			_tcscat_s(m_sError,_T("A disk error occurred while reading the file.")); 
			break;
		case AVIERR_FILEOPEN:		
			_tcscat_s(m_sError,_T("A disk error occurred while opening the file."));
			break;
		case REGDB_E_CLASSNOTREG:		
			_tcscat_s(m_sError,_T("According to the registry, the type of file specified in AVIFileOpen does not have a handler to process it"));
			break;
		}
	...
	if (hr != AVIERR_OK)
	{
		_tcscpy_s(m_sError,_T("AVI Stream creation failed. Check Bitmap info."));
		if (hr==AVIERR_READONLY)
		{
			_tcscat_s(m_sError,_T(" Read only file."));
		}
		return hr;
	}
	...
	if (hr != AVIERR_OK)
	{
		_tcscpy_s(m_sError,_T("AVI Compressed Stream creation failed."));
		
		switch(hr)
		{
		case AVIERR_NOCOMPRESSOR:
			_tcscat_s(m_sError,_T(" A suitable compressor cannot be found."));
				break;
		case AVIERR_MEMORY:
			_tcscat_s(m_sError,_T(" There is not enough memory to complete the operation."));
				break; 
		case AVIERR_UNSUPPORTED:
			_tcscat_s(m_sError,_T("Compression is not supported for this type of data. This error might be returned if you try to compress data that is not audio or video."));
			break;
		}
	...
	if (hr != AVIERR_OK)
	{
		_tcscpy_s(m_sError,_T("AVI Compressed Stream format setting failed."));
